...
func TestHandleImportCrawlerScriptFileEnsuresCrawlerNameTagWhenAutoGenerateDisabled(t *testing.T) {
	ctx := context.Background()
	tmp := t.TempDir()
	cat, err := catalog.Open(filepath.Join(tmp, "catalog.db"))
	if err != nil {
	...
	seeded, err := cat.EnsureCrawlerTag(ctx, "Imported Crawler")
	if err != nil {
		t.Fatalf("seed crawler tag: %v", err)
	}
	if _, err := cat.DeleteTag(ctx, seeded.ID); err != nil {
	...
	if !ok || label != "Imported Crawler" {
		t.Fatalf("lookup tag = %q/%v, want Imported Crawler/true", label, ok)
	}
	...
	for i := range tags {
		if tags[i].Label == "Imported Crawler" {
			found = &tags[i]
			break
		}
	}
	if found == nil {
		t.Fatal("imported crawler tag not found")
	}
	if found.Source != "generated" {
		t.Fatalf("crawler tag source = %q, want generated", found.Source)
	}
	if !found.CrawlerOwned {
		t.Fatal("crawler tag crawlerOwned = false, want true")
	}
...
func TestHandleUpdateTagSavesRulesAndClassifies(t *testing.T) {
	ctx := context.Background()
	cat, err := catalog.Open(t.TempDir() + "/catalog.db")
	if err != nil {
		t.Fatalf("open catalog: %v", err)
	...
	req := requestWithRouteParam(
		...
		strings.NewReader(`{"aliases":["别名"],"matchRules":{"keywords":["custom matching phrase"],"excludes":["ignore"]}}`),
	)
	rr := httptest.NewRecorder()
	...
	if got.Classified != 1 || len(got.Tag.MatchRules.Keywords) != 1 || len(got.Tag.Aliases) != 1 {
		t.Fatalf("response = %#v", got)
	}
